bit32 = { band = function(a,b) return a & b end, lshift = function(a,b) return (a << b) & 0xffffffff end, rshift = function(a,b) return (a & 0xffffffff) >> b end }
--[[
...
local reviewudp = Proto("reviewudp", "UDP transport for RDP")

-- UDP1 fields
local pf_udp_snSourceAck = ProtoField.uint32("reviewudp.snsourceack", "snSourceAck", base.HEX)
local pf_udp_ReceiveWindowSize = ProtoField.uint16("reviewudp.receivewindowsize", "ReceiveWindowSize", base.DEC)
local pf_udp_flags = ProtoField.uint16("reviewudp.flags", "Flags", base.HEX)
...
local pf_udp_flag_syn = ProtoField.bool("reviewudp.flags.syn", "Syn", base.HEX, nil, RDPUDP_SYN)
local pf_udp_flag_fin = ProtoField.bool("reviewudp.flags.fin", "Fin", base.HEX, nil, RDPUDP_FIN)
local pf_udp_flag_ack = ProtoField.bool("reviewudp.flags.ack", "Ack", base.HEX, nil, RDPUDP_ACK)
local pf_udp_flag_data = ProtoField.bool("reviewudp.flags.data", "Data", base.HEX, nil, RDPUDP_DATA)
local pf_udp_flag_fec = ProtoField.bool("reviewudp.flags.fec", "FECData", base.HEX, nil, RDPUDP_FEC)
local pf_udp_flag_cn = ProtoField.bool("reviewudp.flags.cn", "CN", base.HEX, nil, RDPUDP_CN)
local pf_udp_flag_cwr = ProtoField.bool("reviewudp.flags.cwr", "CWR", base.HEX, nil, RDPUDP_CWR)
local pf_udp_flag_aoa = ProtoField.bool("reviewudp.flags.aoa", "Ack of Acks", base.HEX, nil, RDPUDP_AOA)
local pf_udp_flag_synlossy = ProtoField.bool("reviewudp.flags.synlossy", "Syn lossy", base.HEX, nil, RDPUDP_SYNLOSSY)
local pf_udp_flag_ackdelayed = ProtoField.bool("reviewudp.flags.ackdelayed", "Ack delayed", base.HEX, nil, RDPUDP_ACKDELAYED)
local pf_udp_flag_correlationId = ProtoField.bool("reviewudp.flags.correlationid", "Correlation id", base.HEX, nil, RDPUDP_CORRELATIONID)
local pf_udp_flag_synex = ProtoField.bool("reviewudp.flags.synex", "SynEx", base.HEX, nil, RDPUDP_SYNEX)

local pf_udp_snInitialSequenceNumber = ProtoField.uint32("reviewudp.initialsequencenumber", "Initial SequenceNumber", base.HEX)
local pf_udp_upstreamMtu = ProtoField.uint16("reviewudp.upstreammtu", "Upstream MTU", base.DEC)
local pf_udp_downstreamMtu = ProtoField.uint16("reviewudp.downstreammtu", "DownStream MTU", base.DEC)

local pf_udp_correlationId = ProtoField.new("Correlation Id", "reviewudp.correlationid", ftypes.BYTES)

local pf_udp_synex_flags = ProtoField.uint16("reviewudp.synex.flags", "Flags", base.HEX)
local pf_udp_synex_flag_version = ProtoField.bool("reviewudp.synex.flags.versioninfo", "Version info", base.HEX, nil, 0x0001)
local pf_udp_synex_version = ProtoField.uint16("reviewudp.synex.version", "Version", base.HEX, {[1]="Version 1", [2]="Version 2", [0x101]="Version 3"})
local pf_udp_synex_cookiehash = ProtoField.new("Cookie Hash", "reviewudp.synex.cookiehash", ftypes.BYTES)

local pf_udp_ack_vectorsize = ProtoField.uint16("reviewudp.ack.vectorsize", "uAckVectorSize", base.DEC)
local pf_udp_ack_item = ProtoField.uint8("reviewudp.ack.item", "Ack item", base.HEX)
local pf_udp_ack_item_state = ProtoField.uint8("reviewudp.ack.item.state", "VECTOR_ELEMENT_STATE", base.HEX, {[0]="Received", [1]="Reserved 1", [2]="Reserved 2", [3]="Pending"}, 0xc0)
local pf_udp_ack_item_rle = ProtoField.uint8("reviewudp.ack.item.rle", "Run length", base.DEC, nil, 0x3f)

local pf_udp_fec_coded = ProtoField.uint32("reviewudp.fec.coded", "snCoded", base.HEX)
local pf_udp_fec_sourcestart = ProtoField.uint32("reviewudp.fec.sourcestart", "snSourceStart", base.HEX)
local pf_udp_fec_range = ProtoField.uint8("reviewudp.fec.range", "Range", base.DEC)
local pf_udp_fec_fecindex = ProtoField.uint8("reviewudp.fec.fecindex", "Fec index", base.HEX)

local pf_udp_resetseqenum = ProtoField.uint32("reviewudp.resetSeqNum", "snResetSeqNum", base.HEX)

local pf_udp_source_sncoded = ProtoField.uint32("reviewudp.data.sncoded", "snCoded", base.HEX)
local pf_udp_source_snSourceStart = ProtoField.uint32("reviewudp.data.sourceStart", "snSourceStart", base.HEX)


-- UDP2 fields
local pf_PacketPrefixByte = ProtoField.new("PacketPrefixByte", "reviewudp2.prefixbyte", ftypes.UINT8, nil, base.HEX)

local pf_packetType = ProtoField.uint8("reviewudp2.packetType", "PacketType", base.HEX, {[0] = "Data", [8] = "Dummy"}, 0x1e, "type of packet")
...
local pf_flags = ProtoField.uint16("reviewudp2.flags", "Flags", base.HEX, nil, 0xfff, "flags")
local pf_flag_ack = ProtoField.bool("reviewudp2.flags.ack", "Ack", base.HEX, nil, RDPUDP2_ACK, "packet contains Ack payload")
local pf_flag_data = ProtoField.bool("reviewudp2.flags.data", "Data", base.HEX, nil, RDPUDP2_DATA, "packet contains Data payload")
local pf_flag_ackvec = ProtoField.bool("reviewudp2.flags.ackvec", "AckVec", base.HEX, nil, RDPUDP2_ACKVEC, "packet contains AckVec payload")
local pf_flag_aoa = ProtoField.bool("reviewudp2.flags.ackofacks", "AckOfAcks", base.HEX, nil, RDPUDP2_AOA, "packet contains AckOfAcks payload")
local pf_flag_overhead = ProtoField.bool("reviewudp2.flags.overheadsize", "OverheadSize", base.HEX, nil, RDPUDP2_OVERHEAD, "packet contains OverheadSize payload")
local pf_flag_delayackinfo = ProtoField.bool("reviewudp2.flags.delayackinfo", "DelayedAckInfo", base.HEX, nil, RDPUDP2_DELAYACK, "packet contains DelayedAckInfo payload")

local pf_logWindow = ProtoField.uint16("reviewudp2.logWindow", "LogWindow", base.DEC, nil, 0xf000, "flags")

local pf_AckSeq = ProtoField.uint16("reviewudp2.ack.seqnum", "Base Seq", base.HEX)
local pf_AckTs = ProtoField.uint24("reviewudp2.ack.ts", "receivedTS", base.DEC)
local pf_AckSendTimeGap = ProtoField.uint8("reviewudp2.ack.sendTimeGap", "sendTimeGap", base.DEC)
local pf_ndelayedAcks = ProtoField.uint8("reviewudp2.ack.numDelayedAcks", "NumDelayedAcks", base.DEC, nil, 0x0f)
local pf_delayedTimeScale = ProtoField.uint8("reviewudp2.ack.delayedTimeScale", "delayedTimeScale", base.DEC, nil, 0xf0)
local pf_delayedAcks = ProtoField.new("Delayed acks", "reviewudp2.ack.delayedAcks", ftypes.BYTES)
local pf_delayedAck = ProtoField.uint8("reviewudp2.ack.delayedAck", "Delayed ack", base.DEC)

local pf_OverHeadSize = ProtoField.uint8("reviewudp2.overheadsize", "Overhead size", base.DEC)

local pf_DelayAckMax = ProtoField.uint8("reviewudp2.delayackinfo.max", "MaxDelayedAcks", base.DEC)
local pf_DelayAckTimeout = ProtoField.uint8("reviewudp2.delayackinfo.timeout", "DelayedAckTimeoutInMs", base.DEC)

local pf_AckOfAcks = ProtoField.uint16("reviewudp2.ackofacks", "Ack of Acks", base.HEX)

local pf_DataSeqNumber = ProtoField.uint16("reviewudp2.data.seqnum", "sequence number", base.HEX)
local pf_DataChannelSeqNumber = ProtoField.uint16("reviewudp2.data.channelseqnumber", "Channel sequence number", base.HEX)
local pf_Data = ProtoField.new("Data", "reviewudp2.data", ftypes.BYTES)

local pf_AckvecBaseSeq = ProtoField.uint16("reviewudp2.ackvec.baseseqnum", "Base sequence number", base.HEX)
local pf_AckvecCodecAckVecSize = ProtoField.uint16("reviewudp2.ackvec.codecackvecsize", "Codec ackvec size", base.DEC, nil, 0x7f)
local pf_AckvecHaveTs = ProtoField.bool("reviewudp2.ackvec.havets", "have timestamp", base.DEC, nil, 0x80)
local pf_AckvecTimeStamp = ProtoField.uint24("reviewudp2.ackvec.timestamp", "Timestamp", base.HEX)
local pf_AckvecCodedAck = ProtoField.uint8("reviewudp2.ackvec.codecAck", "Coded Ack", base.HEX)
local pf_AckvecCodedAckMode = ProtoField.uint8("reviewudp2.ackvec.codecAckMode", "Mode", base.HEX, {[0]="Bitmap", [1]="Run length"}, 0x80)
local pf_AckvecCodedAckRleState = ProtoField.uint8("reviewudp2.ackvec.codecAckRleState", "State", base.HEX, {[0]="lost",[1]="received"}, 0x40)
local pf_AckvecCodedAckRleLen = ProtoField.uint8("reviewudp2.ackvec.codecAckRleLen", "Length", base.DEC, nil, 0x3f)
...
reviewudp.prefs.track_udp2_peer_states = Pref.bool("Track state of UDP2 peers", true, "Keep track of state of UDP2 peers (receiver and sender windows")
reviewudp.prefs.debug_ssl = Pref.bool("SSL debug message", false, "print verbose message of the SSL fragments reassembly")
	


local field_reviewudp_flags = Field.new("reviewudp.flags")
local field_reviewudp2_packetType = Field.new("reviewudp2.packetType")
local field_reviewudp2_channelSeqNumber = Field.new("reviewudp2.data.channelseqnumber")
local field_reviewudp2_ackvec_base = Field.new("reviewudp2.ackvec.baseseqnum")
...
function dissectV2(in_tvbuf, pktinfo, tree)
	...
	if reviewudp.prefs.track_udp2_peer_states then
	...
	local dataTree
	local isDummy = (field_reviewudp2_packetType()() == 0x8)
	...
	if bit32.band(flags, RDPUDP2_ACKVEC) ~= 0 then
		...
		local seqNumber = field_reviewudp2_ackvec_base()()
...
function reviewudp.dissector(in_tvbuf, pktinfo, root)
	-- dprint2("reviewudp.dissector called")
	pktinfo.cols.protocol:set("RDP-UDP")
	
	local pktlen = in_tvbuf:reported_length_remaining()
	local tree = root:add(reviewudp, in_tvbuf:range(0,pktlen))
...
DissectorTable.get("udp.port"):add(3389, reviewudp)

...
if args[1] == "permissions" { print("canPostMouseEvents=\(CGPreflightPostEventAccess())") } else if args[1] == "windows" {
 ...
 CGEvent(mouseEventSource:CGEventSource(stateID:.hidSystemState),mouseType:.mouseMoved,mouseCursorPosition:p,mouseButton:.left)!.post(tap:.cgSessionEventTap); Thread.sleep(forTimeInterval:0.3); report("move")
}

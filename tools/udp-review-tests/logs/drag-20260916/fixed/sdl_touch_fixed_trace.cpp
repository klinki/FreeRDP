...
bool SdlTouch::handleEvent(SdlContext* sdl, const SDL_MouseMotionEvent& ev)
{
	...
	struct timeval stamp{}; gettimeofday(&stamp, nullptr);
	float gx=0,gy=0; SDL_GetGlobalMouseState(&gx,&gy);
	fprintf(stderr, "DRAG_FINAL %.6f age_ms=%.3f wid=%u state=%u relative=%d x=%d y=%d global=%.2f,%.2f\n", stamp.tv_sec+stamp.tv_usec/1e6, (double)(SDL_GetTicksNS()-ev.timestamp)/1e6, ev.windowID, ev.state, relative, x,y,gx,gy);
...
bool SdlTouch::handleEvent(SdlContext* sdl, const SDL_MouseButtonEvent& ev)
{
	...
	struct timeval stamp{}; gettimeofday(&stamp, nullptr);
	float gx=0,gy=0; SDL_GetGlobalMouseState(&gx,&gy);
	fprintf(stderr, "DRAG_BUTTON %.6f wid=%u down=%d button=%u relative=%d x=%d y=%d global=%.2f,%.2f\n", stamp.tv_sec+stamp.tv_usec/1e6, ev.windowID, ev.down, ev.button, relative, x,y,gx,gy);
